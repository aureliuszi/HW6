...
## We've provided the function access_third_elem.
# Write a lambda function that does exactly the same thing as access_third_elem (you can assume any input it would receive has at least 3 elements in it).
...
# Use the Python map function on the programmers list you just created,
# in order to create an map instance iterator of numbers representing the productivity of each student. Save the map iterator in a variable called prod_iter.
...
# Save the list of tuples in a variable called names_and_productivities. To do this, you should use a list comprehension (you may also use the zip function,
# and you may use any variables you have already created).

## But be careful that if you use answers from previous problems, you use the LISTs you generated,
# so that all your tests can still pass and you avoid confusion!

names_and_productivities=zip(names,prod_list)
names_and_productivities=list(names_and_productivities)
...
names_with_not_too_much_seniority_1 = list ((filter (lambda x: len(x.name) > x.years_UM, programmers)))

names_with_not_too_much_seniority=[item.name for item in names_with_not_too_much_seniority_1]
